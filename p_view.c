...
Auto pitching on slopes?
  ...
  damage = deltavelocity*deltavelocity  * 0.0001f
...
//ZOID
	...
	if (level.time - ent->client->ctf_grapplereleasetime <= FRAMETIME * 2 || // todo: framenum
...
void P_WorldEffects (void)
{
	...
	if (!old_waterlevel && waterlevel)
	{
		...
		current_player->damage_debounce_framenum = level.framenum - 1 * BASE_FRAMERATE; // todo: / BASE_FRAMERATE ?
	}
...
void B_WorldEffects (edict_t *ent)
{
	...
	if (ent->movetype == MOVETYPE_NOCLIP)
	{
		ent->air_finished_framenum = level.framenum + 12 * BASE_FRAMERATE; // don't need air
		return;
	}
